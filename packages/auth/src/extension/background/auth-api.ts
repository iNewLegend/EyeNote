...
export async function extensionSignInWithGoogle () : Promise<{ success : boolean; user ?: AuthUser; error ?: string }> {
    if ( !oauthClientId ) {
...
export async function extensionSignOutUser () : Promise<{ success : boolean; error ?: string }> {
    try {
        await clearAuthSession();
...
export async function extensionGetAuthStatus () : Promise<{ isAuthenticated : boolean; user ?: AuthUser | null }> {
