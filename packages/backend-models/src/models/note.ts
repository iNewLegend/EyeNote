...
export type NoteDocument = Document<unknown, unknown, NoteBase> & NoteBase & {
    userId : string;
    createdAt : Date;
    updatedAt : Date;
};
...
const noteSchemaDefinition : Record<string, unknown> = {
...
export function getNoteModel () : Model<NoteDocument> {
    return models.Note || model<NoteDocument>( "Note", noteSchema );
}
