...
class CommandsManager {
    ...
    public run(id: DCommandIdArgs, args: DCommandArgs, callback?: (result: any) => any) {
        ...
        if (!command) {
            throw new Error(
                `Command '${commandName}' not registered for component '${componentName}'`
            );
        }
    ...
    public hook(
        id: DCommandIdArgs,
        callback: (result?: any, args?: DCommandArgs) => any,
        options?: {
        ...
        const singleComponentContext = core[GET_INTERNAL_SYMBOL](
            componentNameUnique
        ) as DCommandSingleComponentContext;

        // Check if id exist within the component context
        if (!singleComponentContext.commands[commandName]) {
            throw new Error(
                `Command '${commandName}' not registered for component '${componentNameUnique}'`
            );
        }
    ...
    public unhookWithinComponent(componentNameUnique: string) {
        ...
        singleComponentContext &&
            Object.keys(singleComponentContext.commands).forEach((commandName) => {
