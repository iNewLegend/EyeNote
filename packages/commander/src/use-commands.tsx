...
import type { DCommandArgs, DCommandComponentContextProps } from "./definitions";
...
export function useCommanderCommand(commandName: string) {
    ...
    return {
        run: (args: DCommandArgs, callback?: (result: any) => void) =>
            commandsManager.run(id, args, callback),
        hook: (callback: (result: any, args?: DCommandArgs) => void) =>
            commandsManager.hook(id, callback),
 ...
 */
export function useCommanderComponent(
    ...
    return {
        ...
        getContext: () => context!,
        getState: <TState extends React.ComponentState>() =>
            core[GET_INTERNAL_SYMBOL](id).getState() as TState,
    };
}
...
export function useCommanderChildrenComponents(
    ...
    const [childrenComponents, setChildrenComponents] = React.useState<
        ReturnType<typeof useCommanderComponent>[]
    >([]);
    ...
    React.useEffect(() => {
        ...
        if (!children) {
            throw new Error(
                `Current component: '${componentContext.getComponentName()}' cannot find: '${componentName}' children`
            );
        }
