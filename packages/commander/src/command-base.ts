...
import type { DCommandOptions, DCommandArgs, DCommandRegisterArgs } from "./definitions";
...
export abstract class CommandBase<TState = React.ComponentState, TArgs = DCommandArgs> {
    ...
    public static getName(): string {
        throw new Error(
            "You have should implement `static getName()` method, since the commands run by name ;)"
        );
    }
    ...
    private validateState() {
        if (
            "undefined" === typeof this.options.state ||
            "function" !== typeof this.options.setState
        ) {
            throw new Error(
                "There is no state for the current command, you should use `withCommands( component, class, state, commands )` including the state to enable it"
            );
        }
    }
}
