...
import { ComponentIdProvider } from "./commands-provider";
...
export function withCommands(
    ...
    let commands: DCommandNewInstanceWithArgs[], state: React.ComponentState;
    ...
    const WrappedComponent = class WrappedComponent extends React.PureComponent<
        any,
    ...
    > {
        ...
        public componentWillUnmount() {
            ...
            if (this.$$commander.lifecycleHandlers[INTERNAL_ON_UNMOUNT]) {
                this.$$commander.lifecycleHandlers[INTERNAL_ON_UNMOUNT](
                    core[GET_INTERNAL_SYMBOL](this.context.getNameUnique())
                );
            }
        ...
        public componentDidMount() {
            ...
            if (this.$$commander.lifecycleHandlers[INTERNAL_ON_MOUNT]) {
                this.$$commander.lifecycleHandlers[INTERNAL_ON_MOUNT](
                    core[GET_INTERNAL_SYMBOL](this.context.getNameUnique())
                );
            }
        }
        ...
        public render() {
            if (this.$$commander.lifecycleHandlers[INTERNAL_ON_LOAD]) {
                this.$$commander.lifecycleHandlers[INTERNAL_ON_LOAD](
                    core[GET_INTERNAL_SYMBOL](this.context.getNameUnique())
                );
            }
     ...
     **/
    function handleAncestorContexts(
        context: DCommandComponentContextProps,
        parentContext: DCommandComponentContextProps
    ) {
