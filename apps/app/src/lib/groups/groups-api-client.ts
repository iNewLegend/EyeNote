...
export async function leaveGroup ( groupId : string ) : Promise<{ group : GroupRecord; left : boolean }> {
    return apiRequest<{ group : GroupRecord; left : boolean }>( `/api/groups/${ groupId }/leave`, {
        method: "POST",
    } );
}
